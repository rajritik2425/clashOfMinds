          ...
          datasets: [
            {
              ...
              backgroundColor: ["#facc15", "#f97316", "#ef4444", "#a855f7", "#3b82f6", "#10b981"],
              borderColor: "#fde047",
              borderWidth: 1,
            },
          ],
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-blue-900 text-white p-4 md:p-8 font-sans">
      ...
      <div className="flex flex-wrap gap-3 justify-center mb-8">
        <Button
          ...
          className={`px-4 py-2 rounded-lg ${
            selectedTopic === null ? "bg-yellow-500 text-gray-900 font-bold" : "bg-gray-800 hover:bg-gray-700"
          }`}
        >
          All Topics
        </Button>
        {Object.keys(topicMistakeCount).map((topic) => (
          <Button
            ...
            className={`px-4 py-2 rounded-lg ${
              selectedTopic === topic ? "bg-yellow-500 text-gray-900 font-bold" : "bg-gray-800 hover:bg-gray-700"
            }`}
          >
      ...
      {activeTab === "questions" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
          {filteredQuestions.map((q, i) => (
            <div
              key={q.id}
              className="bg-gradient-to-br from-gray-800 to-gray-900 border-2 border-yellow-500 rounded-xl p-5 shadow-lg hover:shadow-yellow-500/20 transition-all"
            >
              <div className="flex justify-between items-start mb-3">
                <div className="bg-yellow-500 text-gray-900 px-3 py-1 rounded-full text-sm font-bold">
                  ⚔️ BATTLE #{i + 1}
                </div>
                <div className="bg-red-500 text-white px-3 py-1 rounded-full text-sm">{q.topic}</div>
              </div>
              ...
              <div className="bg-gradient-to-r from-purple-900 to-blue-900 p-4 rounded-lg border border-purple-500">
                <p className="text-yellow-200">
                  <span className="font-bold">🎯 Strategy Tip:</span> Revise <strong>{q.topic}</strong> with specialized
                  practice. Unlock the <strong className="text-yellow-400">Elite Troop Challenge</strong> for bonus XP!
                </p>
              </div>
            </div>
          ))}
      ...
      {activeTab === "battleplan" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          {Object.entries(topicMistakeCount)
            .sort((a, b) => b[1] - a[1])
            .map(([topic, count]) => (
              <div
                key={topic}
                className="bg-gradient-to-br from-gray-800 to-gray-900 border-2 border-blue-500 rounded-xl p-5 shadow-lg"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          {Object.entries(topicMistakeCount)
            ...
            .map(([topic, count]) => (
              <div
                key={topic}
                className="bg-gradient-to-br from-gray-800 to-gray-900 border-2 border-red-500 rounded-xl p-5 shadow-lg hover:shadow-red-500/20 transition-all"
              >
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gradient-to-r from-blue-900 to-blue-800 p-4 rounded-lg border border-blue-500">
            ...
            <p className="text-gray-300">
              Focus your training on high-mistake areas to strengthen your defenses against these topics. Allocate 60%
              of your study time to the top three weakness areas.
            </p>
          </div>
          ...
          <div className="bg-gradient-to-r from-purple-900 to-purple-800 p-4 rounded-lg border border-purple-500">
            ...
            <p className="text-gray-300">
              Launch targeted attacks on your weaknesses by completing Elite Challenges. Each challenge conquered earns
              you XP and brings you closer to total academic domination!
            </p>
          </div>
        </div>
      </div>
